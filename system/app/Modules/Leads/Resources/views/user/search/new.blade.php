        ...
        <div class="srch">
            ...
            <div class="srch__work">
                        ...
                        <div class="srch__recall-col">
                            ...
                            <div class="srch__recall-list">
                                @forelse ($savedSearches ?? [] as $savedSearch)
                                    <button type="button" class="srch__recall-item text-left" data-search-preset='@json($savedSearch->filters, JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_TAG | JSON_HEX_QUOT)'>
                                        <i class="ph ph-magnifying-glass" aria-hidden="true"></i>
                        ...
                        <div class="srch__recall-col">
                            ...
                            <div class="srch__recall-list">
                                @forelse ($recentSearches ?? [] as $recentSearch)
                                    <button type="button" class="srch__recall-item text-left" data-search-preset='@json($recentSearch->filters, JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_TAG | JSON_HEX_QUOT)'>
                ...
                <section class="srch__results{{ isset($results) ? '' : ' is-hidden' }}" data-search-results>
                    @if (isset($searchRun) && $searchRun->status === \App\Modules\Leads\Models\SearchRun::STATUS_FAILED)
                        <x-ui.alert type="error" class="mb-4">
                            {{ $searchRun->error_message ?: __('The search failed. Check the Google Maps API settings and logs.') }}
                        </x-ui.alert>
                    @endif
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                document.querySelectorAll('[data-search-preset]').forEach((button) => {
                    button.addEventListener('click', () => {
                        try {
                            ...
                            console.warn('Unable to load search preset.', error);
                        }
                    });
                });
                ...
                document.getElementById('save-name')?.addEventListener('keydown', (event) => {
                    ...
                    document.querySelector('[data-save-search-submit]')?.click();
                });
            });
        </script>
    @endpush
