/* Lead scoring — weight sliders re-score a sample of your leads in place. */
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Reset the sliders to their defaults once the confirm dialog is accepted.
  const resetBtn = form.querySelector('[type="reset"]');
  if (resetBtn) {
    const defaults = Object.fromEntries(
      weights.map((el) => [el.dataset.weight, el.getAttribute("value")]),
    );
    resetBtn.addEventListener("confirm:accepted", () => {
      weights.forEach((el) => {
        el.value = defaults[el.dataset.weight] ?? (el.min || 0);
      });
      render();
    });
  }
