...
<section class="spy-section" data-anim>
    <div class="container">
        <div class="max-w-[42rem]" data-anim-item>
            <p class="stage-head__key stage-head__key--find">
                <i class="{{ $data['key_icon'] ?? 'ph-fill ph-map-pin' }}" aria-hidden="true"></i>
                {{ $data['key_label'] ?? '' }}
            </p>
        ...
        <div class="@container mt-12 grid gap-5 md:mt-14 sm:grid-cols-2 lg:grid-cols-4" data-anim-item>
            @foreach($items as $item)
                <article class="rounded-xl border border-neutral-200 bg-neutral-0 p-5">
                    <span class="flex h-10 w-10 items-center justify-center rounded-lg text-[1.25rem] fcard__icon--find">
